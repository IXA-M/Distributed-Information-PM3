...
8n/workflows/file-upload-notification.json. It integrates three microservices:
- **audit-log**: records the upload event.
- **central-logging**: writes a structured log entry.
- **Notification mock**: sends an alert.

To use it, import the JSON into an n8n instance running in the Kubernetes cluster (see k8s/n8n.yaml for deployment). The webhook trigger can be called by any service (e.g., Upload Session) to start the automation.
